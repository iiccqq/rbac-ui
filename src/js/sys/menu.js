...
var vm = new Vue({
    ...
    methods: {
        getMenu: function(menuId){
            //加载菜单树
            $.get(baseURL + "sys/users/"+userid+"/menustree", function(r){
        ...
        update: function () {
            ...
            $.get(baseURL + "sys/users/" + userid +"/menus/"+menuId, function(r){
                vm.showList = false;
                vm.title = "修改";
                vm.menu = r.menu;
        ...
        del: function() {
        	...
        	confirm('确定要删除选中的记录？', function() {
        		$.ajax({
        			type: "DELETE",
        			url: baseURL + "sys/users/" + userid + "/menus/" + menuId,        		
        			success: function(r) {
        				if(r.code === 0) {
        					alert('操作成功', function() {
        						vm.reload();
...
$(function () {
    ...
    var table = new TreeTable(Menu.id, baseURL + "sys/users/" + userid +"/menus", colunms);
